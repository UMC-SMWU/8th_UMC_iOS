...
final class ButtonInfoList {
    ...
    static let buttonList: [ButtonInfo] = [
        .init(title: "GET", action: {
            ...
            serviceManager.getUserData(name: "망고")
        }),
        .init(title: "POST", action: {
            serviceManager.createUser(.init(name: "망고", age: 23, address: "서울시", height: 171))
        }),
        .init(title: "PATCH", action: {
            serviceManager.updateUserPatch(.init(name: nil, age: 23, address: nil, height: nil))
        }),
        .init(title: "PUT", action: {
            serviceManager.updateUserPut(.init(name: "망고", age: 23, address: "서울시", height: 171))
        }),
        .init(title: "DELETE", action: {
            serviceManager.deleteUser(name: "망고")
        }),
    ]
}
